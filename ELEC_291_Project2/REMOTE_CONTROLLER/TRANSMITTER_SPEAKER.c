...
{
    unsigned int i;
    unsigned int cycles = ms * 4;
    for (i = 0; i < cycles; i++)
    {
        SPEAKER_OUT = 0;
        lcd_delay_us(244);   // 244us = 2048Hz, same as your alarm clock lab
        SPEAKER_OUT = 1;
        lcd_delay_us(244);
    }
    SPEAKER_OUT = 1;
...
// Alarm clock pattern: BEEP BEEP BEEP
void beep_alarm(void)
{
    beep_tone(80);   lcd_delay_ms(80);   // BEEP
    beep_tone(80);   lcd_delay_ms(80);   // BEEP
    beep_tone(80);                        // BEEP
}

// Short confirm stays the same
void beep_confirm(void) { beep_tone(40); }

// Reset = alarm pattern
void beep_reset(void)   { beep_alarm(); }
...
char _c51_external_startup (void)
{
    ...
    P0MDOUT |= 0b_0000_1100; // P0.2 (haptic) and P0.3 (speaker) push-pull
